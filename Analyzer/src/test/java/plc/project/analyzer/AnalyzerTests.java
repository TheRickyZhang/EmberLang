...
final class AnalyzerTests {
    ...
    private static void test(Input input, @Nullable Ir expected, ParserMethod<? extends Ast> method) {
        ...
        try {
            ...
            if (expected == null) {
                Assertions.fail("Expected an exception to be thrown, but produced IR of type: " + ir.getClass().getSimpleName());
            }
            Assertions.assertEquals(expected, ir);
        } catch (AnalyzeException e) {
            if (expected != null) {
                System.out.println("HUh");
                Assertions.fail("Unexpected AnalyzeException thrown (" + e.getMessage() + "), expected " + ".");
            }
        }
    }
}
