...
public final class Analyzer implements Ast.Visitor<Ir, AnalyzeException> {
    ...
    public Ir.Stmt.Def visit(Ast.Stmt.Def ast) throws AnalyzeException {
        ...
        for(int i = 0; i < sz; ++i) {
            funcScope.define(paramNames.get(i),paramTypes.get(i));
        }
        // VERIFY
        funcScope.define("$RETURNS", returnType);
    ...
    public Ir.Stmt.Return visit(Ast.Stmt.Return ast) throws AnalyzeException {
        Optional<Type> expectedType = scope.get("$RETURNS", true);
    ...
    public Ir.Expr.Method visit(Ast.Expr.Method ast) throws AnalyzeException {
        ...
        List<Ast.Expr> astArgs = ast.arguments();
        if (astArgs.size() != parameters.size()) {
            throw new AnalyzeException("Method " + name + " expects " + parameters.size() + " args lol bozo");
        }
        for (int i = 0; i < astArgs.size(); i++) {
            Ir.Expr arg = visit(ast.arguments().get(i));
            requireSubtype(arg.type(), parameters.get(i));
    ...
    // This freaking thing - AHHHHHH so hard to debug stack overflow -> misaligned scope / object hashcode? Like huh?
    @Override
    public Ir.Expr.ObjectExpr visit(Ast.Expr.ObjectExpr ast) throws AnalyzeException {
        ...
        try {
            ...
            // FIRST pre declare all methods in the object scope.
            Set<String> methodNames = new HashSet<>();
            for (Ast.Stmt.Def methodAst : ast.methods()) {
                String methodName = methodAst.name();
                if (fieldNames.contains(methodName) || !methodNames.add(methodName)) {
                    throw new AnalyzeException("Method " + methodName + " conflicts with " + name);
                }
                // FOR TEST 3, NEED to do this, think intuitively HOW we've moved the implicit "this" definition
                objectScope.define(methodName, new Type.Function(List.of(), Type.ANY));
            }
            ...
            for (Ast.Stmt.Def methodAst : ast.methods()) {
                ...
                methodScope.define("this", objectType); // The ever-elusive "this"

                Scope savedScope = scope;
                scope = methodScope;
                try {
                    methods.add(visit(methodAst));
                } finally {
                    scope = savedScope;
                }
            }
