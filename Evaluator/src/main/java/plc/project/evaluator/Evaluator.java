...
public final class Evaluator implements Ast.Visitor<RuntimeValue, EvaluateException> {
    ...
    public RuntimeValue visit(Ast.Stmt.For ast) throws EvaluateException {
        // Unavoidable unchecked warning; since we erase generics we suppress even though should be known???
        @SuppressWarnings("unchecked")
        List<RuntimeValue> list = requireType(visit(ast.expression()), List.class);

        // Think about WHY we can't just declare scope = new Scope(scope) outside this loop
        // If we have (for x : a) { x++; print(x); } then x should be reset every time with new scope, matching values in a
    ...
    public RuntimeValue visit(Ast.Expr.Literal ast) throws EvaluateException {
        // System.out.println("LITERAL " + System.identityHashCode(scope));
    ...
    public RuntimeValue visit(Ast.Expr.Binary ast) throws EvaluateException {
        ...
        switch (op) {
            ...
            case "==", "!=" -> {
                ...
                boolean eq = Objects.equals(left, right);
                return new RuntimeValue.Primitive(eq == op.equals("=="));
            }
            case "<", "<=", ">", ">=" -> {
                Object left = requireType(x, Object.class);
                Object right = requireType(y, Object.class);
